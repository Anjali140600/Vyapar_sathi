...
class RAGService:
    def __init__(self, db_folder=str(DEFAULT_DB_FOLDER)):
    ...
    def query(self, query_text: str, n_results=5) -> list:
        ...
        exact_docs = []
        if re.fullmatch(
            r"\s*(?:what\s+is|define|meaning\s+of|explain)\s+(?:the\s+)?gst[?.!\s]*",
            clean_query,
        ):
            clean_query = (
                "GST definition unified indirect tax applied on the supply "
                "of goods and services in India"
            )
        elif re.search(
            r"\b(?:mobile\s+phones?|mobile\s+handsets?|cell\s+phones?|smartphones?)\b",
            clean_query,
        ):
            exact_result = self.collection.get(
                where={"hsn": "8517"},
                include=["documents"],
            )
            exact_docs = [
                "Mobile phones and mobile handsets use HSN 8517 for this "
                f"GST rate lookup. {document}"
                for document in exact_result.get("documents", [])
            ]
        ...
        docs = exact_docs + [doc for doc in semantic_docs if doc not in exact_docs]
