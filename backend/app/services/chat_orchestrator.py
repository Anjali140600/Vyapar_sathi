...
class ChatOrchestrator:
    ...
    def process_input(self, text: str = None, image_path: str = None, audio_path: str = None, user_id: str = "default_user") -> str:
        ...
        if q_type == "general":
            if self._should_use_llm_general(final_query, docs):
    ...
    def _should_use_llm_general(self, query: str, docs: list[str]) -> bool:
    ...
    def _answer_rag_query(self, final_query: str, docs: list[str], prefer_single_line: bool = False) -> str:
        ...
        generated_answer = self.llm.generate_rag_response(
            final_query,
            docs,
            max_lines=max_lines,
            max_words=55 if prefer_single_line else 90,
        )
        if generated_answer:
            return generated_answer
        return self.rag_service.build_concise_answer(
            final_query, docs, max_lines=max_lines
        )
    ...
    def _generate_fallback(self, q_type, data, context, extracted_data=None, final_query: str = ""):
        ...
        if q_type == "mixed":
            ...
            if not answer:
                docs = [doc for doc in context.split("\n") if doc.strip()]
                rag_answer = self._answer_rag_query(
                    final_query, docs, prefer_single_line=True
                ) if docs else ""
