...
from backend.app.models.schema import UserDocument, User, Conversation
...
structured_ocr_service = StructuredOCRService()
...
def run_ocr(
    ...
    regex_result = ocr_service.process_image(doc.file_path)
    result = structured_ocr_service.enhance(regex_result)
...
async def input_image(
    ...
    try:
        ...
        with open(img_path, "wb") as f:
            f.write(image_bytes)
        ocr = OCRService()
        regex_result = ocr.process_image(img_path)
        result = structured_ocr_service.enhance(regex_result)
    finally:
...
def multimodal_health(current_user: User = Depends(get_current_user)):
    ...
    from backend.app.services.stt_service import _ffmpeg_path
